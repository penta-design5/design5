...
| P8 | 마스킹 프리셋 (원형 절단) | 🟡 진행중 | `refactor/phase2-api-layer` → `origin/2026-06-17-tiper` | **P8-1~P8-6 코드 완료.** 개발망 `migrate deploy` 적용 + 프리셋 왕복·유니크·cascade·legacy 무영향 확인 ✅ / tsc·lint 0 + 단위테스트 228 통과(신규 30종) + `?tab=plus` 200. P8-5(앵커 기준 코너·설정 여부 시각화)·P8-6(코너 점 클릭으로 프리셋 편집 진입)까지 반영. **남은 것은 개발망 육안 검증**(계획서 §10). 계획서: [ICON_PLUS_절단마스킹_구현계획.md](./ICON_PLUS_절단마스킹_구현계획.md) |
...
- 다음 작업:
  ...
  - P8-5 확인 포인트: 우측 상단에서 **높이가 다른 병합 리소스들의 아래쪽 변이 앵커에 정렬**되는지, 탭·카드 표시로 설정 여부가 한눈에 구분되는지

#### P8-6 메인 카드 진입 방식 정리 (편집 버튼 제거 → 코너 점 클릭)  ✅ (코드 완료 · 개발망 육안 확인 대기)
...
## 후속 기능 — P8 마스킹 프리셋 (2026-07-29 계획 수립, **P8-1~P8-6 코드 완료 · 개발망 육안 검증 대기**)
...
- 단계: ~~P8-1 스키마·마이그레이션·API~~ ✅ → ~~P8-2 마스크 코어~~ ✅ → ~~P8-3 사용자 경로~~ ✅ → ~~P8-4 관리자 편집 다이얼로그~~ ✅ → ~~P8-5 개발망 피드백(앵커 기준 코너·설정 여부 시각화)~~ ✅ → ~~P8-6 코너 점 클릭 진입~~ ✅ → **개발망 육안 검증**(계획서 §10 2~13번)
