...
| P8 | 마스킹 프리셋 (원형 절단) | ✅ 완료 | `refactor/phase2-api-layer` → `origin/2026-06-17-tiper` | **P8-1~P8-6 완료 + 개발망 동작 확인(2026-07-30)** ✅. `migrate deploy` 적용 + 프리셋 왕복·유니크·cascade·legacy 무영향 확인, tsc·lint 0 + 단위테스트 233 통과(신규 37종), 개발망에서 관리자 프리셋 설정 → 사용자 프리셋 전환 → 마스킹 결과까지 확인. 잔여는 코드 작업이 아닌 운영 점검 항목(아래 잔여 목록). 계획서: [ICON_PLUS_절단마스킹_구현계획.md](./ICON_PLUS_절단마스킹_구현계획.md) |
...
#### P8-5 개발망 피드백 반영 (앵커 기준 코너 + 설정 여부 시각화)  ✅
...
- **메인 카드**(관리자): 프리셋 표시를 **카드의 실제 코너**(우측 상단/우측 하단)에 점으로 배치 → 위치 설명 없이 "어디에 구멍이 설정됐는지"가 바로 읽힌다. 개발망 2·3차 피드백을 거쳐 **코너 점이 편집 진입로 자체**가 되었다(§P8-6).
...
#### 개발망 검증 (2026-07-30)  ✅
- **확인 완료**: 완전한 아이콘 업로드(업로드 폼에 anchor 입력 없음) → 카드 우측 코너 점으로 프리셋 편집 진입 → 원 드래그·지름·앵커 설정 후 저장 → 사용자 속성 패널에서 **프리셋 전환에 따라 마스킹 결과가 바뀌는 것**까지 정상 동작. P8-5(위치별 앵커 기준 코너)·P8-6(코너 점 클릭 진입) 반영 결과도 확인.
- **P8-1 데이터 계층은 별도 확인 완료**: `migrate deploy` 적용, 프리셋 2개 저장/1개 교체/전체 삭제 왕복, `(resourceId, position)` 중복 거부(P2002), MAIN 삭제 시 cascade, 기존 MAIN 5건 legacy anchor 보존.
- **잔여 점검 항목**(코드 작업 아님, 계획서 §10): 마스킹 결과의 **SVG/PNG/JPG 3포맷 파일 일치·투명도**, **Safari 마스크 렌더**, 카드에서 프리셋 보유 MAIN **삭제 시 cascade 육안 확인**, **비관리자 403 + 코너 점 미노출**, 기존 pre-cut MAIN의 **legacy 회귀**.

#### P8-6 메인 카드 진입 방식 정리 (편집 버튼 제거 → 코너 점 클릭)  ✅
> 2026-07-29 개발망 3차 피드백. 편집 버튼과 프리셋 표시가 카드 우측에 몰려 겹쳐 보이던 문제에서 출발해,
> **"코너 점을 누르면 그 위치의 프리셋 탭이 열린다"** 로 진입로를 하나로 합쳤다.

- **별도 편집(⌖) 버튼 제거.** 카드 우측 코너의 점 2개가 유일한 편집 진입로다.
- 코너 점 클릭 → 해당 위치 프리셋 탭으로 다이얼로그 오픈. **미설정 코너를 누르면 그 위치에 기본 프리셋 draft가 생성**되어 "빈 코너를 눌러 구멍을 만든다"는 흐름이 된다(저장 전이라 취소/삭제로 되돌림).
- 표시 규칙: 카드 hover/포커스 시 두 점이 함께 나타난다(설정=채운 점, 미설정=점선 빈 점). **hover가 없는 기기(터치)는 항상 표시** — 그렇지 않으면 관리자의 진입로가 사라진다.
- 클릭 표적은 8px 점이 아니라 **20px 버튼**(점은 그 안에 렌더). 점 위에서 커서가 포인터로 바뀌고 점이 확대+링으로 강조된다. **툴팁은 사용하지 않고**(사용자 결정) 접근성 이름은 `aria-label`(`우측 상단 프리셋 편집/추가`)로 제공한다.
- 편집 버튼이 없어진 만큼 **메인 섹션 설명문을 관리자에게만** `카드 우측 코너의 점을 눌러 마스킹 프리셋을 설정합니다`로 바꿔 진입 방법을 안내한다.
- 수정 파일: `CutPositionGlyph.tsx`(코너 점을 버튼으로), `IconPlusCard.tsx`(편집 버튼 제거, `onEdit(id, position)`), `ResourceSection.tsx`(prop 시그니처), `IconPlusMainEditDialog.tsx`(`initialPosition` prop + 초기화 로직), `app/_category-pages/icon/IconPlusWorkspace.tsx`(편집 state를 `{id, position}`으로)
- 계획 대비 변경/결정:
  - 다이얼로그 초기화 effect 의존성에 `initialPosition`을 포함했다. 같은 아이콘의 **다른 코너를 눌러 다시 열 때** `resource` 참조가 동일해 탭이 갱신되지 않는 문제가 있었다.
  - 코너 점을 `<button>`으로 만들되 선택 버튼과 **형제**로 유지해 버튼 중첩을 피했다(기존 편집 버튼과 동일 구조).
  - 카드 우측 코너 20px 두 곳은 카드 선택 영역에서 빠진다. 기존 편집 버튼이 이미 우측 상단 28px를 차지했으므로 실질 손실은 없다.
- 검증: tsc/lint 0 + vitest 233 통과 + `/icon?tab=plus` 200. Tailwind **이름 있는 group(`group/dot:hover`)과 `@media(hover:none)` 규칙이 생성 CSS에 실제로 방출되는지 확인**(클래스명만 믿지 않음 — 과거 `tailwindcss-animate` 미등록으로 전 페이지 애니메이션이 no-op이던 사례 때문)
...
## 후속 기능 구현 완료 (2026-07-30)
- **P8 마스킹 프리셋(P8-1~P8-6) 구현 완료 + 개발망 동작 확인.** 메인 아이콘을 완전한 모습으로 다루고, 관리자가 위치별 (절단 원 + 앵커) 프리셋을 설정하면 사용자가 속성 패널에서 위치를 골라 마스킹된 결과를 받는다. 원본 SVG는 훼손하지 않는다.
- 기존 pre-cut + 단일 anchor 방식은 **legacy 경로로 공존**한다(일괄 전환 시점은 운영 판단).

## 후속 기능 — P8 마스킹 프리셋 (2026-07-29 계획 수립, **2026-07-30 구현·개발망 확인 완료** ✅)
...
- 단계: ~~P8-1 스키마·마이그레이션·API~~ ✅ → ~~P8-2 마스크 코어~~ ✅ → ~~P8-3 사용자 경로~~ ✅ → ~~P8-4 관리자 편집 다이얼로그~~ ✅ → ~~P8-5 개발망 피드백(앵커 기준 코너·설정 여부 시각화)~~ ✅ → ~~P8-6 코너 점 클릭 진입~~ ✅ → ~~개발망 동작 확인~~ ✅ (2026-07-30)
...
- **개발망 확인 완료(2026-07-30)**: **P8 마스킹 프리셋 전체 흐름**(완전한 아이콘 업로드 → 코너 점으로 프리셋 편집 → 사용자 프리셋 전환 → 마스킹 결과) + P8-1 데이터 계층(마이그레이션·프리셋 왕복·유니크·cascade).
- **잔여 운영 확인**: P2 관리자 업로드/삭제·비관리자 403(API), P3 실 데이터 카드 표시·선택·삭제, P4 위험 SVG 차단, **P8 마스킹 결과의 3포맷 다운로드 일치·Safari 마스크 렌더·프리셋 보유 MAIN 삭제(cascade) 육안·비관리자 코너 점 미노출·legacy pre-cut 회귀** 최종 확인. (P1 검증 완료)
  - ※ 기존 항목이던 "MAIN anchor 재편집(hover 편집→PATCH)"은 **P8에서 마스킹 프리셋 편집으로 대체**되어 종료. legacy anchor 값은 DB·API에 남아 프리셋 없는 아이콘의 렌더에 계속 쓰인다.
...
| 2026-07-29 | P8-5 | **개발망 1차 피드백 2건 반영**: ①**위치별 앵커 기준 코너** — `merge-svg`에 `anchorBasis`(기본 TOP_LEFT로 기존 출력 불변) 추가, 우측 상단은 앵커에 리소스 **좌하단**을 맞춰 높이가 다른 배지들의 아래쪽 변이 정렬된다. 매핑은 DB 없이 코드 상수(`CUT_POSITION_ANCHOR_BASIS`)로 결정. 다이얼로그의 기본 앵커·`맞추기` 버튼·참조 오버레이·안내문 동반 수정. ②**설정 여부 시각화** — `CutPositionGlyph`(미니 다이어그램) 신규로 다이얼로그 탭(글리프+✓/+·실선/점선·`추가` 라벨, 활성은 ring). **메인 카드는 편집 버튼을 좌측 상단으로 옮기고 프리셋 점을 카드의 실제 코너(우측 상단/하단)에 배치**(설정=항상 표시, 미설정=hover 시 점선) — 2차 피드백 반영. tsc/lint 0 + **233 통과**(신규 5종) + `?tab=plus` 200. ⚠️ 기존 우측 상단 프리셋은 `앵커를 원 좌하단에 맞추기` 1회로 재조정 필요 |
| 2026-07-29 | P8-6 | **메인 카드 진입 방식 정리**(개발망 3차 피드백): 좌측 상단 편집(⌖) 버튼을 **제거**하고 **우측 코너 점 클릭 → 해당 위치 프리셋 탭으로 다이얼로그 오픈**으로 진입로를 통합. 미설정 코너 클릭 시 기본 draft 생성. 점은 카드 hover/포커스 시 노출(**터치 기기는 항상 노출** — 진입로 소실 방지), 20px 클릭 표적 + 커서 포인터 + hover 확대·링, **툴팁 미사용**(사용자 결정, `aria-label`로 대체). 다이얼로그에 `initialPosition` 추가(같은 아이콘의 다른 코너 재오픈 시 탭 갱신), 편집 state를 `{id, position}`으로. 관리자용 섹션 설명문으로 진입 방법 안내. tsc/lint 0 + 233 통과 + 생성 CSS에서 `group/dot:hover`·`@media(hover:none)` 방출 확인 |
| 2026-07-30 | P8 | **개발망 동작 확인 완료 → P8 ✅.** 완전한 아이콘 업로드 → 카드 코너 점으로 프리셋 편집 → 사용자 프리셋 전환 → 마스킹 결과까지 정상. P8-1~P8-6(스키마·API / 마스크 코어 / 사용자 경로 / 관리자 다이얼로그 / 앵커 기준 코너·설정 여부 시각화 / 코너 점 진입) 전부 반영. 잔여는 코드 작업이 아닌 운영 점검(3포맷 다운로드 일치·Safari·cascade 육안·비관리자 403·legacy 회귀). 문서 상태 갱신 |
