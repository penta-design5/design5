...
- **메인 카드**(관리자): 편집(⌖) 버튼을 **좌측 상단**으로 옮기고, 프리셋 표시를 **카드의 실제 코너**(우측 상단/우측 하단)에 점으로 배치 → 위치 설명 없이 "어디에 구멍이 설정됐는지"가 바로 읽힌다. 설정된 위치는 항상 채워진 점, **미설정 위치는 hover/포커스 시에만 점선 빈 점**(평소 목록은 깔끔, 관리자가 올리면 빈 자리 확인). `title`/`sr-only`로 접근성 텍스트 제공.
  - 개발망 2차 피드백 반영(2026-07-29): 편집 버튼과 프리셋 표시가 모두 우측에 몰려 겹쳐 보이던 문제 해결. Tailwind `group-hover`/`group-focus-within` 방출은 생성 CSS에서 직접 확인.
