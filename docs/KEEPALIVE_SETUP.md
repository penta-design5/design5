...
(이 프로젝트는 커스텀 도메인 **layerary.com** 사용 중이므로, Value에는 `https://layerary.com` 을 넣으면 됩니다.)
...
배포가 끝난 뒤 브라우저나 curl로 `https://layerary.com/api/keepalive` 에 접속해 보면,  
보안을 설정하지 않았다면 `{"ok":true,"db":true,"supabase":true,"at":"..."}` 같은 JSON이 보여야 합니다.  
