...
- 최종 업데이트: 2026-07-13 (P0~P6 완료 ✅ + 사내망 확인, P7만 대기 · 후속 3건 **모두 사내망 확인 완료 ✅**: ①업로드 드래그 앤 드롭 ②카드 프리셋 색=하단 3px 테두리 액센트 ③`cardColor` 마이그레이션 개발망 적용) · 푸시: `origin/2026-06-17-tiper` (최신 `14fe577`)
- **다음 세션 시작점: P7 (반응형/권한/QA 최종 점검)** — 아래 "Phase별 상세 > Phase 7" 체크리스트부터 진행. P0~P6 코드+UI + 후속 3건 모두 사내망 검증 완료.
  - ⚠️ **운영 배포 시 주의**: `cardColor` 마이그레이션(`20260713120000_add_insight_card_color`)은 **개발망만 적용됨** — 운영망(design5/192.168.1.42) 배포 시 `prisma migrate deploy` 반드시 실행(안 하면 insight_posts 쿼리 전체 실패, 2026-07-13 인시던트 참조).
...
| P7 | 반응형/권한/QA + 최종 검증 | 🟡 진행중 | `refactor/phase2-api-layer` | 코드 감사(권한/반응형) 완료 + 모바일 타이틀 정렬·제목 CSS 말줄임 수정. tsc 0 / lint 0. 실기기 수동 QA는 개발망 대기 |
...
- [ ] (개발망 수동 QA) 모바일 사이드바(Sheet)·카드 그리드·게시판 테이블 실기기 육안
- [ ] (개발망 수동 QA) 권한 매트릭스 실동작(미로그인/일반/관리자 × 조회·업로드·수정·삭제)
- [ ] (개발망 수동 QA) iframe `sandbox` 동작·문서 스타일 격리 (다크모드는 프로젝트 미지원 확정 — 제외)
- [ ] 최종 `tsc --noEmit` 0 / `next lint` 0 (현재 두 수정 반영 후 0 확인 ✅, 배포 전 재확인)
- 완료 기준: 데스크톱/모바일 회귀 없음, 권한 게이팅 정상
- 검증: 로컬 컴파일/타입/린트(완료) + 개발망 데스크톱·모바일 수동 QA(대기)
- 다음: 배포 (운영 DB 카테고리 2건 시드/수동 삽입 → `migrate deploy`(cardColor 포함) → `origin/2026-06-17-tiper` 반영)

---

## 후속 수정 (2026-07-13) — P7 모바일 타이틀 정렬 + 게시판 제목 CSS 말줄임

최신 동향 페이지 사내망 피드백 2건 수정 (`app/_category-pages/insights-trend/InsightTrendListPage.tsx` 단일 파일).
...
- **검증**: `tsc --noEmit` 0 / `next lint`(해당 파일) 0. 실기기 육안은 개발망 권장.
