...
- 최종 업데이트: **2026-08-05** (AI 사용가이드 카드 높이 행 단위 정렬(masonry → CSS Grid) + 짧은 설명 카드 장식 아이콘 + 리사이즈 FLIP 애니메이션 복구 — **개발망 확인 완료 ✅**) · 푸시: `origin/2026-06-17-tiper` (최신 `23006ff`)
  - 이전: 2026-07-13 (**P0~P7 전 단계 완료 ✅ + 사내망 확인** · P7 후속 UI 4건 모두 사내망 확인 완료 ✅: ①모바일 타이틀 좌측정렬 ②게시판 제목 CSS 말줄임 ③좁은 폭 제목 컬럼 가독성(게시일 축소·패딩·전체 탭영역) ④다이얼로그 헤더 모바일 좌측정렬), 커밋 `10f83ec`
- **다음 세션 시작점: 운영 배포** — P0~P7 코드+UI + 사내망 QA + 2026-08-05 카드 레이아웃 개선까지 모두 완료. 남은 것은 운영망 반영뿐. (2026-08-05 변경은 **DB/스키마 변경 없음** — 추가 마이그레이션 불필요)
  - ⚠️ **운영 배포 시 주의**: `cardColor` 마이그레이션(`20260713120000_add_insight_card_color`)은 **개발망만 적용됨** — 운영망(design5/192.168.1.42) 배포 시 `prisma migrate deploy` 반드시 실행(안 하면 insight_posts 쿼리 전체 실패, 2026-07-13 인시던트 참조). 또한 운영 DB에 INSIGHTS 카테고리 2건(`ai-guide`/`latest-trends`) 시드/삽입 필요.
...
| P7 | 반응형/권한/QA + 최종 검증 | ✅ 완료 | `refactor/phase2-api-layer` → `origin/2026-06-17-tiper` (10f83ec) | 코드 감사(권한/반응형) + 후속 UI 4건 수정. tsc 0 / lint 0(기존 무관 경고 2건 제외). **사내망 수동 QA 완료 ✅** |
...
## 후속 수정 (2026-07-13) — AI 사용가이드 카드 프리셋 색 적용 방식 변경(하단 테두리)

> **2026-07-13 재조정(사내망 피드백)**: 배경 그라데이션에 프리셋 색을 넣으니 너무 강해서, **배경은 기존 흰색→`#F7F8FA` 그라데이션으로 고정**하고 프리셋 색은 **카드 하단 테두리(3px 액센트)** 로 적용하도록 변경. 색 미선택(기본)은 강조 없이 균일 1px 테두리 유지 → 색을 고른 카드만 하단 액센트로 구분. `cardColor` 컬럼/저장 방식·API·프리셋 8색은 그대로(표현만 변경). 수정 파일: `InsightGuideCard.tsx`(배경 복원 + 조건부 `borderBottom 3px`), `InsightPostFormDialog.tsx`(피커 스와치를 "은은한 배경 + 3px 하단 테두리" 미니 카드 미리보기로, 라벨 "카드 하단 테두리 색"). tsc 0 / lint 0. DB 변경 없음. **사내망 확인 완료(2026-07-13) ✅ — "완벽".** (미해결 여지: 모서리 3px 테두리가 거슬리면 `box-shadow: inset 0 -3px 0`, 굵기 조정 가능 — 현재 피드백은 만족.)
>
> (참고: `rounded-lg` 모서리에서 3px 하단 테두리가 곡선을 살짝 타므로, 거슬리면 `box-shadow: inset 0 -3px 0` 방식으로 전환 가능.)

아래는 최초 도입(배경 그라데이션 방식) 기록 — 데이터 모델/마이그레이션은 그대로 유효.

AI 사용가이드 게시물 등록/수정 시 **카드에 프리셋 색을 지정**하는 기능. 프리셋은 Penta Design System의 **Success/Warning/Error/Info 8색**(각 500·700):
...
## 후속 수정 (2026-08-05) — AI 사용가이드 카드 높이 정렬(masonry → CSS Grid) + 장식 아이콘

AI 사용가이드 목록에서 **설명 길이에 따라 카드 높이가 제각각이라 정렬이 깨져 보이던 문제**를 해결하고, 짧은 설명 카드의 빈 공간에 장식 아이콘을 추가했다. 커밋 `a8c9a40` → `a543711` → `23006ff`. **개발망 확인 완료 ✅ — "완벽".**

수정 파일 3개: `app/_category-pages/insights-guide/InsightGuideListPage.tsx`, `components/insights/InsightGuideCard.tsx`, `lib/insights-schemas.ts`. **DB/스키마 변경 없음**(운영 배포 시 추가 마이그레이션 불필요).

### ① 카드 높이 — 행 단위 정렬 (`a8c9a40`)

- **원인**: 카드 높이가 `min-h-[200px]` 위로 내용에 따라 자유롭게 늘어났다. 실측하면 세 변수(제목 1/2줄 × 33px, 설명 0~5줄 × 22.75px + `mt-3`, 컬러 액센트 시 하단 테두리 1px→3px)의 조합으로 **200 / 210.75 / 243.75px** 등으로 갈렸다.
- **판단**: "전역 고정 높이(244px)"도 검토했으나, 설명이 짧은 카드의 하단 여백이 과도해져 **행 단위 최대 높이**로 결정.
- **수정**: masonry(세로 컬럼) → **CSS Grid** 전환. grid 기본값 `align-items: stretch` 덕에 한 행의 카드가 그 행에서 가장 높은 카드에 자동으로 맞춰진다. masonry는 DOM에 "행" 개념이 없어(각 컬럼이 별도 부모) 구조적으로 불가능했던 정렬.
  - 부수 효과로 `columns` state / `calculateColumns()` / resize 리스너 / `containerRef` 제거 → **JS 컬럼 계산 로직이 통째로 사라짐**(코드 순감).
  - 카드 배열 순서가 세로(컬럼) → 가로(행)로 바뀌어 **목록 순서와 읽기 순서가 일치**(개선).
  - 카드에 `h-full` 부여, `Flipped` 래퍼 `<div>`에도 `h-full`(grid 아이템이 행 높이를 카드에 전달).
- **영향 범위 없음**: `globals.css`의 `.masonry-container`/`.masonry-column`은 **건드리지 않았다.** HW·다이어그램·캐릭터·데스크톱·웰컴보드·EDM·CI/BI·PPT·갤러리·`GenericListPage` 등 다른 목록은 기존 masonry 그대로. `InsightGuideCard`/`InsightGuideListPage`는 `pageType === 'insights-guide'` 한 곳에서만 쓰인다(최신 동향은 테이블이라 무관).

### ② 짧은 설명 카드 장식 아이콘 (`a8c9a40`)

- 설명이 **한 줄 이하**(없는 경우 포함)인 카드 우하단에 lucide `BookOpenText` 64px, `opacity: 0.15`.
- 색은 카드 선택색(`cardColor`), 미선택 시 **신규 상수 `INSIGHT_CARD_ICON_NEUTRAL_COLOR = '#94A3B8'`**. 기존 `INSIGHT_CARD_DEFAULT_COLOR`(`#F7F8FA`)는 카드 배경색과 같아 아이콘에 쓰면 보이지 않으므로 별도로 뒀다.
- **1줄 판정은 글자 수 근사**: 카드 내부 텍스트 폭 `320 − 48(p-6) − 2(border) = 270px`, `text-sm`(14px) 기준 한글 약 19자/줄 → 임계값 18, 전각 1자·ASCII 0.5자 가중(`isAtMostOneLine`). ref 실측(`offsetHeight`) 방식은 레이아웃 후 상태 갱신에 따른 깜빡임·웹폰트 로딩 타이밍 문제가 있어 채택하지 않음. 순수 함수라 SSR 안전.
- **겹침 방지**: 아이콘을 텍스트보다 **앞에** 배치하고 `h3`/`p`에 `relative` 부여 → 위치 지정 요소는 DOM 순서대로 그려지므로 본문이 항상 위. `pointer-events-none` + `aria-hidden="true"`. 최악 케이스(제목 2줄 + 설명 1줄 = 152.75px → `min-h` 200px 적용)에서도 64px 아이콘은 본문을 침범하지 않는다.

### ③ 리사이즈 애니메이션 복구 (`a543711` → `23006ff`) — 2회 시도

grid 전환 후 **창 크기를 바꿀 때 카드가 부드럽게 이동하지 않고 순간 이동**하는 회귀가 발생했다. (masonry 라이브러리를 제거한 게 아니다 — `react-masonry-css`는 package.json에 있으나 **프로젝트 어디에서도 import하지 않는다.** 애니메이션은 `react-flip-toolkit`이 담당하며 그대로 유지됨.)

- **1차 시도 `a543711` (불충분)**: `flipKey`가 `items` 순서만 참조해 리사이즈 시 값이 안 바뀌던 문제로 보고, `ResizeObserver`로 컬럼 수를 관측해 `flipKey`에 포함시켰다. → **여전히 애니메이션 없음.**
- **실제 원인 `23006ff`**: `Flipper`는 이전 위치를 **`getSnapshotBeforeUpdate`**(React가 DOM을 바꾸기 직전)에서 측정한다(라이브러리 소스 확인). 그런데 `grid-cols-[repeat(auto-fill,...)]`는 **브라우저가 리사이즈 시점에 이미 재배치를 끝낸다.** ResizeObserver → setState → 리렌더가 도는 시점엔 카드가 벌써 새 위치라 **First == Last, 이동 거리 0**으로 측정되어 애니메이션이 생략됐다.
  - 즉 `flipKey`는 필요조건이었을 뿐, 진짜 원인은 **재배치 주체가 React가 아니라 CSS**라는 점. 이전 masonry가 동작했던 것도 컬럼 분배를 React가 DOM으로 바꿨기 때문이다.
- **수정**: `gridTemplateColumns`를 인라인 스타일로 `repeat(columnCount, 320px)` **명시**. 리사이즈만으로는 트랙 위치가 변하지 않고, React가 `columnCount`를 바꿀 때만 카드가 이동하므로 FLIP이 정상적으로 First/Last 차이를 잡는다. `auto-fill` 클래스는 첫 측정 전(초기 렌더·SSR) 폴백으로만 남김.
- **`translate` 플래그**: `<Flipped translate>`로 **위치만** 애니메이션. 행 단위 stretch 때문에 컬럼 수가 바뀌면 카드 높이도 함께 변하는데, 기본값(translate+scale+opacity)이면 `transform: scale()`이 렌더된 픽셀을 통째로 늘려 **전환 중 글자가 늘어나 보인다**(전환 종료 시 원복되긴 하나 최대 약 1.22배라 눈에 띔).
- **알려진 동작**: 컬럼 수가 바뀌지 않는 범위의 미세 리사이즈는 애니메이션이 없다. 이전 masonry도 동일했으므로 회귀 아님.

### 검증

`npm run typecheck` 0 / `npm run lint` 해당 파일 경고 0. 로컬은 DB 부재로 렌더 확인 불가 → **개발망 검증 통과 ✅**(행 정렬·아이콘 색/겹침·리사이즈 애니메이션·글자 왜곡 없음).

### 남은 여지

- 아이콘 표시 임계값 18자는 근사치다. 게시물이 늘어난 뒤 경계(한글 18~20자)에서 어색한 카드가 보이면 `ONE_LINE_MAX_WIDTH` 조정.
- 컬럼 수 변경 시 카드 **높이**는 애니메이션 없이 즉시 바뀐다. 거슬리면 `inverseFlipId`로 내부 역스케일 래퍼를 한 겹 넣어 크기까지 애니메이션 가능(현 피드백은 만족).
...
- [x] **최신 동향 타이틀 모바일 우측정렬 버그 수정** — 아래 "후속 수정(2026-07-13) — P7 모바일 타이틀 정렬" 참조
- [x] **게시판 제목 CSS 말줄임 전환** — 아래 동일 항목 참조(가로 스크롤 제거)
- [x] **좁은 모바일 폭 제목 컬럼 가독성 개선** — 아래 "후속 수정(2026-07-13) — 좁은 폭 제목 가독성" 참조
- [x] **다이얼로그 헤더 모바일 좌측정렬** — 아래 "후속 수정(2026-07-13) — 다이얼로그 헤더 정렬" 참조
- [x] 권한 매트릭스 **코드 감사**: 미로그인→`/login`, 조회=requireAuth, 업로드/수정/삭제=requireAdmin(P2 401 게이팅+상세 `isAdmin` 버튼). 가이드 카드 액션은 `isAdmin && !isMobileViewport`(HW 패턴), 상세 수정/삭제도 `isAdmin` — 일관 확인 ✅
- [x] 반응형 **코드 감사**: 상세 헤더 `flex-wrap`+`min-w-0`(제목 축소·버튼 랩), 게시판 헤더 `page-header-stack`(모바일 타이틀 좌측정렬 수정 완료), 테이블 `table-fixed`로 가로 스크롤 제거
- [x] (사내망 수동 QA) 모바일 게시판 테이블/타이틀/다이얼로그 정렬·가독성 **사내망 확인 완료 ✅**
- [x] 최종 `tsc --noEmit` 0 / `next lint` 0 (insights·dialog 파일 clean; `lib/svg`·chart의 기존 경고 2건은 본 작업과 무관)
- 완료 기준: 데스크톱/모바일 회귀 없음, 권한 게이팅 정상 — **충족 ✅**
- 검증: 로컬 컴파일/타입/린트(완료) + 사내망 데스크톱·모바일 수동 QA(완료)
- 다음: **운영 배포** (운영 DB 카테고리 2건 시드/수동 삽입 → `migrate deploy`(cardColor 포함) → `origin/2026-06-17-tiper` 반영)

---

## 후속 수정 (2026-07-13) — P7 모바일 타이틀 정렬 + 게시판 제목 CSS 말줄임

최신 동향 페이지 사내망 피드백 2건 수정 (`app/_category-pages/insights-trend/InsightTrendListPage.tsx` 단일 파일). 커밋 `7aa502e`. **사내망 확인 완료 ✅**.

### ① 타이틀이 모바일에서 우측 정렬되어 보이던 문제
- **원인**: 공용 `.page-header-stack`(`globals.css`)은 모바일에서 `flex-col`+`items-end`. 타이틀 블록 `<div>`는 가장 긴 줄 폭으로 줄어들어 `items-end`가 **오른쪽으로 밀어냄**. HW·AI 사용가이드 등은 설명 문구가 **길어 줄바꿈되어 블록이 전체 폭을 차지** → 타이틀이 좌측으로 보였을 뿐, 구조는 동일. 최신 동향은 설명이 짧아 한 줄에 들어가며 블록이 콘텐츠 폭(≈273px)으로 줄어 우측 정렬로 노출됨(콘텐츠 길이 의존적 취약점).
- **판단**: 공용 클래스를 `items-start md:items-end`로 바꾸면 전 페이지 모바일에서 **액션 버튼이 우→좌로 이동**(사내망에서 정상 확인된 다른 페이지가 바뀜) → 회귀로 간주하여 **채택 안 함**.
- **수정**: 최신 동향 타이틀 블록에만 `w-full md:w-auto` 부여 → 모바일에서 블록이 전체 폭 차지(타이틀 좌측), 버튼은 기존대로 우측. 다른 페이지 영향 0. (동일 취약점이 잠재된 `insights-guide`는 현재 설명이 길어 정상이라 미변경 — 필요 시 같은 방식 적용 가능.)

### ② 제목이 길면 테이블 가로 스크롤 발생
- **원인**: 테이블이 `[&_td]:whitespace-nowrap`로 줄바꿈 금지 + 제목은 JS 40자 슬라이스만 적용 → 한글 40자도 넓어 컨테이너를 넘겨 `overflow-auto` 가로 스크롤 유발.
- **수정**: `<Table>`에 `table-fixed` + 제목 셀 `truncate`(CSS `overflow:hidden`+`text-ellipsis`+`nowrap`)로 **가용 폭에 맞춰 자동 말줄임** → 어떤 화면폭에서도 가로 스크롤 없음. JS 슬라이스(`trendTitleDisplay`) 제거, 링크는 항상 **전체 제목** 렌더(CSS가 시각 처리). 40자 초과 시 전체 제목 툴팁 유지(`TITLE_TOOLTIP_MIN`).
- **검증**: `tsc --noEmit` 0 / `next lint`(해당 파일) 0. **사내망 확인 완료 ✅**.

---

## 후속 수정 (2026-07-13) — 좁은 폭 제목 가독성

게시판 제목이 CSS 말줄임으로 가로 스크롤은 사라졌으나, **아주 좁은 모바일 폭**에서 게시일 컬럼(고정 `w-40`=160px)이 폭을 과점해 제목이 한 글자만 보이는 문제. `InsightTrendListPage.tsx`, 커밋 `d390bc3`. **사내망 확인 완료 ✅**.
- **① 게시일 컬럼 축소**: `w-40` → `w-[68px] md:w-40` + 모바일 날짜 축약(`formatTrendDate`: 데스크톱 `"2026. 7. 9."` / 모바일 `"26.7.9."`).
- **② 셀 좌우 패딩 축소**: 테이블 셀 `px-2 md:px-4`(기본 `p-4`의 좌우를 모바일에서 절반으로) → 실사용 폭 확보.
- **③ 탭 영역 확대**: 제목 링크 `block w-full` → 제목이 좁게 잘려도 **셀 전체가 클릭(탭) 영역**. `truncate`를 셀에서 링크로 이동(말줄임 동작 동일).
- (더 좁은 320px급 대비: 필요 시 "모바일에서 게시일 컬럼 제거 → 제목 아래 2줄" 패턴 여지 있음 — 현 피드백은 만족.)

---

## 후속 수정 (2026-07-13) — 다이얼로그 헤더 정렬

게시물 등록/수정·바탕화면 추가 등 다이얼로그의 제목/설명이 **모바일에서 중앙 정렬**되던 문제. `components/ui/dialog.tsx`, 커밋 `10f83ec`. **사내망 확인 완료 ✅**.
- **원인**: shadcn 공용 `DialogHeader` 기본값 `text-center sm:text-left`(모바일 <640px 중앙).
- **수정**: `text-left`로 변경 → 전 화면폭 좌측 정렬. **`Dialog` 계열 전체에 일관 적용**(회귀 위험 없는 개선), 삭제 확인 등에 쓰는 `AlertDialog`는 별도 컴포넌트라 영향 없음.

---

## 리스크 · 미결 사항
> **상태(2026-08-05):** P0~P7 구현·사내망 QA 완료 + 2026-08-05 카드 레이아웃 개선 개발망 확인 완료. 아래 항목은 대부분 해소됨 — 남은 실질 리스크는 **운영 배포 절차**뿐. (2026-08-05 변경은 DB 무관이라 배포 절차에 영향 없음)
- ~~**개발망 의존 검증:**~~ ✅ 해소 — 업로드/DB/iframe 렌더·권한·반응형 모두 사내망(개발망) 확인 완료.
- ~~**iframe 뷰어 방식 확정 필요(P2/P3):**~~ ✅ 확정 — 프록시 뷰어 라우트(`/[id]/view`) 방식으로 사내망 정상 동작. 직접 S3 URL 대안은 미채택.
- ~~**iframe 높이:**~~ ✅ 확정 — 컨테이너 높이(`h-[calc(100dvh-220px)] min-h-[480px]`)로 확정.
- **HTML 크기 상한:** 5MB 가정 유지 — 실제 운영 문서가 더 크면 `lib/insights-storage.ts` 상한 조정.
- **⚠️ 운영 배포(미완료):** ①운영 DB에 INSIGHTS 카테고리 2건(`ai-guide`/`latest-trends`) 시드·삽입 ②`prisma migrate deploy`로 `add_insights` + `add_insight_card_color` 적용(안 하면 insight_posts 쿼리 전체 실패) ③운영망(design5/192.168.1.42) 배포. 상세는 상단 "운영 배포 시 주의" 참조.
