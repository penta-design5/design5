...
#### Phase 3 사내망 dev 검증 체크리스트 (코드 완료분 — 동작 보존 확인)
분리·통합은 배럴/공개 메서드명을 유지해 **동작 동일**하도록 했으나, UI·실파일 결과는 사내망에서 눈으로 확인 필요:
- [ ] **다이어그램 에디터**: 도형 생성/이동/정렬(Transformer bbox), **PNG·JPG·SVG·PPTX 내보내기** 결과물이 기존과 동일.
- [ ] **ICON 메뉴**: 아이콘 미리보기 색상/두께/크기 변경, 다운로드 SVG 정상.
- [ ] **CI·BI 메뉴**: Penta/기타 색상 세트 적용, 다운로드 정상.
- [ ] **CARD 에디터**: 프리셋 저장/불러오기/삭제, 자동저장 복원, 내보내기 파일명.
...
## 5. 새 세션 시작 멘트(예시)
> "REFACTORING_HANDOFF.md 기준으로 Phase 3 잔여(서버 스토리지 진입점 통합 + `extractKeyFromPublicUrl` dedup)를
>  사내망 dev 런타임 검증과 함께 진행해줘." (§3 Phase 3 ⏸ 보류 항목 참조)
