...
const ISIGN_LANGUAGES = ['EN', 'KR', 'JP'] as const

const isignPostSchema = z.object({
  title: z.string().min(1, '제목을 입력해주세요.'),
  description: z.string().optional().nullable(),
  type: z.enum(ISIGN_TYPES, {
    required_error: '타입을 선택해주세요.',
  }),
  language: z.enum(ISIGN_LANGUAGES, {
...
type IsignPostFormValues = z.infer<typeof isignPostSchema>
...
interface IsignUploadDialogProps {
  open: boolean
  onClose: () => void
  categorySlug: string
  categoryId: string
...
export function IsignUploadDialog({
  open,
  onClose,
  categorySlug,
  categoryId,
  ...
  const form = useForm<IsignPostFormValues>({
    resolver: zodResolver(isignPostSchema),
    defaultValues: {
      title: '',
      description: null,
      type: 'iSIGN',
  ...
  useEffect(() => {
    if (isEditMode && post) {
      ...
      const existingType = post.concept && ISIGN_TYPES.includes(post.concept as any)
        ? post.concept
        : 'iSIGN'

      // 기존 Post의 tool 필드에서 언어 추출
      const existingLanguage = post.tool && ISIGN_LANGUAGES.includes(post.tool as any)
        ? post.tool
        : 'EN'
      ...
      form.reset({
        ...
        type: existingType as typeof ISIGN_TYPES[number],
        language: existingLanguage as typeof ISIGN_LANGUAGES[number],
  ...
  const onSubmit = async (values: IsignPostFormValues) => {
    // 중복 제출 방지
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEditMode ? 'iSIGN 게시물 수정' : 'iSIGN 게시물 추가'}
          </DialogTitle>
          <DialogDescription>
