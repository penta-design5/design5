...
export function DiagramZipSection() {
  ...
  useEffect(() => {
    const fetchZipInfo = async () => {
      try {
        const response = await fetch('/api/diagrams/zip')
        if (response.ok) {
          const data = await response.json()
          setZipInfo(data.zipInfo)
        }
      } catch (error) {
        console.error('Failed to fetch diagram ZIP info:', error)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/posts/files?url=${encodeURIComponent(zipInfo.url)}`
      )
      if (!response.ok) {
          ...
          {isAdmin && (
            <div>
              ...
              <Button
                ...
                className="w-full"
                onClick={() =>
                  document.getElementById('diagram-zip-file-input')?.click()
                }
                disabled={uploading}
              >
                {uploading ? (
