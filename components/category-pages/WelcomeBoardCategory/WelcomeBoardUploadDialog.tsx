...
const WELCOMEBOARD_LANGUAGES = ['EN', 'KR', 'JP']

const welcomeBoardPostSchema = z.object({
...
type WelcomeBoardPostFormValues = z.infer<typeof welcomeBoardPostSchema>
...
interface WelcomeBoardUploadDialogProps {
  open: boolean
  onClose: () => void
  onSuccess: () => void
  categoryId: string
...
export function WelcomeBoardUploadDialog({
  open,
  onClose,
  onSuccess,
  categoryId,
  ...
  const form = useForm<WelcomeBoardPostFormValues>({
    resolver: zodResolver(welcomeBoardPostSchema),
    defaultValues: {
      title: '',
      description: '',
      type: '가로',
  ...
  const onSubmit = async (values: WelcomeBoardPostFormValues) => {
    // 중복 제출 방지
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEditMode ? '웰컴보드 게시물 수정' : '웰컴보드 게시물 추가'}
          </DialogTitle>
          <DialogDescription>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      ...
                      <SelectContent>
                        {WELCOMEBOARD_LANGUAGES.map((lang) => (
