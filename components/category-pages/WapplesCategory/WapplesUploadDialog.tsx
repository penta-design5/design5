...
const WAPPLES_LANGUAGES = ['EN', 'KR', 'JP'] as const

const wapplesPostSchema = z.object({
  title: z.string().min(1, '제목을 입력해주세요.'),
  description: z.string().optional().nullable(),
  type: z.enum(WAPPLES_TYPES, {
    required_error: '타입을 선택해주세요.',
  }),
  language: z.enum(WAPPLES_LANGUAGES, {
...
type WapplesPostFormValues = z.infer<typeof wapplesPostSchema>
...
interface WapplesUploadDialogProps {
  open: boolean
  onClose: () => void
  categorySlug: string
  categoryId: string
...
export function WapplesUploadDialog({
  open,
  onClose,
  categorySlug,
  categoryId,
  ...
  const form = useForm<WapplesPostFormValues>({
    resolver: zodResolver(wapplesPostSchema),
    defaultValues: {
      title: '',
      description: null,
      type: 'WAPPLES',
  ...
  useEffect(() => {
    if (isEditMode && post) {
      ...
      const existingType = post.concept && WAPPLES_TYPES.includes(post.concept as any)
        ? post.concept
        : 'WAPPLES'

      // 기존 Post의 tool 필드에서 언어 추출
      const existingLanguage = post.tool && WAPPLES_LANGUAGES.includes(post.tool as any)
        ? post.tool
        : 'EN'
      ...
      form.reset({
        ...
        type: existingType as typeof WAPPLES_TYPES[number],
        language: existingLanguage as typeof WAPPLES_LANGUAGES[number],
  ...
  const onSubmit = async (values: WapplesPostFormValues) => {
    // 중복 제출 방지
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEditMode ? 'WAPPLES 게시물 수정' : 'WAPPLES 게시물 추가'}
          </DialogTitle>
          <DialogDescription>
