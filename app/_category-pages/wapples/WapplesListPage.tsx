...
import { WapplesUploadDialog } from '@/components/category-pages/WapplesCategory/WapplesUploadDialog'
import { WapplesCard } from '@/components/category-pages/WapplesCategory/WapplesCard'
import { WapplesPropertyPanel } from '@/components/category-pages/WapplesCategory/WapplesPropertyPanel'
...
interface Post {
  ...
  fileUrl?: string
  concept?: string | null // 타입 (WAPPLES, WAPPLES CC 등)
  tool?: string | null // 언어 (EN, KR, JP)
  producedAt?: Date | null
  tags?: Array<{ tag: { id: string; name: string; slug: string } }>
}

// ****************************************************************************** WAPPLES 카드 고정 너비 (이 값만 수정하면 됨)
const WAPPLES_CARD_WIDTH = 320

const WAPPLES_FILTERS = [
  'ALL',
  'WAPPLES',
  'WAPPLES CC',
  'WAPPLES SA',
...
export function WapplesListPage({ category }: WapplesListPageProps) {
  ...
  const calculateColumns = useCallback(() => {
    ...
    const cardWidth = WAPPLES_CARD_WIDTH
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      console.error('[WapplesListPage] Download error:', error)
        ...
        <div className="px-8 pt-16 pb-8">
          ...
          {loading && posts.length === 0 && (
            <div ref={containerRef} className="masonry-container justify-center md:justify-start">
              {Array.from({ length: Math.min(4, Math.max(1, Math.floor((containerRef.current?.offsetWidth || 1200) / (WAPPLES_CARD_WIDTH + 8)))) }).map((_, colIndex) => (
                <div key={colIndex} className="masonry-column" style={{ flex: `0 0 ${WAPPLES_CARD_WIDTH}px`, width: `${WAPPLES_CARD_WIDTH}px`, gap: '8px' }}>
                  {Array.from({ length: 3 }).map((_, index) => (
                    <PostCardSkeleton key={index} width={WAPPLES_CARD_WIDTH} height={230} showButtons={true} />
                  ))}
                </div>
              ))}
            </div>
            ...
            >
              <div ref={containerRef} className="masonry-container justify-center md:justify-start">
                {columns.map((column, columnIndex) => (
                  <div key={columnIndex} className="masonry-column" style={{ flex: `0 0 ${WAPPLES_CARD_WIDTH}px`, width: `${WAPPLES_CARD_WIDTH}px`, gap: '8px' }}>
                    {column.map((post) => (
