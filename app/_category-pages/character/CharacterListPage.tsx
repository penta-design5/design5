...
import { CharacterPropertyPanel } from '@/components/category-pages/CharacterCategory/CharacterPropertyPanel'
...
// ****************************************************************************** 캐릭터 카드 고정 너비 (이 값만 수정하면 됨)
const CHARACTER_CARD_WIDTH = 360
...
export function CharacterListPage({ category }: CharacterListPageProps) {
  ...
  const calculateColumns = useCallback(() => {
    ...
    const cardWidth = CHARACTER_CARD_WIDTH
  ...
  const generateDownloadFileName = (format: 'png' | 'jpg' | 'svg'): string => {
    ...
    const sizeStr = width && height ? `_${width}x${height}` : width ? `_${width}w` : ''
  ...
  const handleDownload = async (format: 'png' | 'jpg' | 'svg') => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/posts/${selectedPost.id}/character/download?${queryParams.toString()}`
      )

      if (!response.ok) {
        const errorText = await response.text()
        console.error('[CharacterListPage] 다운로드 에러 응답:', errorText)
      ...
      console.error('[CharacterListPage] Download error:', error)
