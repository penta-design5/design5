...
import { WelcomeBoardUploadDialog } from '@/components/category-pages/WelcomeBoardCategory/WelcomeBoardUploadDialog'
import { WelcomeBoardCard } from '@/components/category-pages/WelcomeBoardCategory/WelcomeBoardCard'
import { WelcomeBoardPropertyPanel } from '@/components/category-pages/WelcomeBoardCategory/WelcomeBoardPropertyPanel'
...
interface WelcomeBoardListPageProps {
  category: Category
}
...
// 웰컴보드 카드 고정 너비
const WELCOMEBOARD_CARD_WIDTH = 320

export function WelcomeBoardListPage({ category }: WelcomeBoardListPageProps) {
  ...
  const calculateColumns = useCallback(() => {
    ...
    const cardWidth = WELCOMEBOARD_CARD_WIDTH
    const gap = 8
  ...
  const flipKey = columns.length > 0 
    ? `welcomeboard:${columns.map((col, idx) => `${idx}:${col.map(p => p.id).join(',')}`).join('|')}`
    : 'empty'
  ...
  return (
    <div className="w-full h-full flex absolute inset-0 bg-neutral-50 dark:bg-neutral-900">
        ...
        <div className="px-8 pt-16 pb-8">
          ...
          {loading && posts.length === 0 && (
            <div ref={containerRef} className="masonry-container justify-center md:justify-start">
              {Array.from({ length: Math.min(4, Math.max(1, Math.floor((containerRef.current?.offsetWidth || 1200) / (WELCOMEBOARD_CARD_WIDTH + 8)))) }).map((_, colIndex) => (
                <div key={colIndex} className="masonry-column" style={{ flex: `0 0 ${WELCOMEBOARD_CARD_WIDTH}px`, width: `${WELCOMEBOARD_CARD_WIDTH}px`, gap: '8px' }}>
                  {Array.from({ length: 3 }).map((_, index) => (
                    <PostCardSkeleton key={index} width={WELCOMEBOARD_CARD_WIDTH} height={230} showButtons={true} />
                  ))}
                </div>
              ))}
            </div>
              ...
              <div ref={containerRef} className="masonry-container justify-center md:justify-start">
                {columns.map((column, colIndex) => (
                  <div
                    ...
                    style={{ flex: `0 0 ${WELCOMEBOARD_CARD_WIDTH}px`, width: `${WELCOMEBOARD_CARD_WIDTH}px`, gap: '8px' }}
                  >
                    {column.map((post) => {
      ...
      {/* 우측: 속성 패널 */}
      <WelcomeBoardPropertyPanel
        post={selectedPost}
        onDownload={handleDownload}
      />

      {/* 업로드 다이얼로그 */}
      <WelcomeBoardUploadDialog
