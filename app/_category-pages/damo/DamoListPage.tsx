...
import { DamoPropertyPanel } from '@/components/category-pages/DamoCategory/DamoPropertyPanel'
...
// ****************************************************************************** D.AMO 카드 고정 너비 (이 값만 수정하면 됨)
const DAMO_CARD_WIDTH = 320
...
export function DamoListPage({ category }: DamoListPageProps) {
  ...
  const calculateColumns = useCallback(() => {
    ...
    const cardWidth = DAMO_CARD_WIDTH
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      console.error('[DamoListPage] Download error:', error)
        ...
        <div className="px-8 pt-16 pb-8">
          ...
          {loading && posts.length === 0 && (
            <div ref={containerRef} className="masonry-container justify-center md:justify-start">
              {Array.from({ length: Math.min(4, Math.max(1, Math.floor((containerRef.current?.offsetWidth || 1200) / (DAMO_CARD_WIDTH + 8)))) }).map((_, colIndex) => (
                <div key={colIndex} className="masonry-column" style={{ flex: `0 0 ${DAMO_CARD_WIDTH}px`, width: `${DAMO_CARD_WIDTH}px`, gap: '8px' }}>
                  {Array.from({ length: 3 }).map((_, index) => (
                    <PostCardSkeleton key={index} width={DAMO_CARD_WIDTH} height={230} showButtons={true} />
                  ))}
                </div>
              ))}
            </div>
            ...
            >
              <div ref={containerRef} className="masonry-container justify-center md:justify-start">
                {columns.map((column, columnIndex) => (
                  <div key={columnIndex} className="masonry-column" style={{ flex: `0 0 ${DAMO_CARD_WIDTH}px`, width: `${DAMO_CARD_WIDTH}px`, gap: '8px' }}>
                    {column.map((post) => (
